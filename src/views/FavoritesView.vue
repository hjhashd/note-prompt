...
const favoritePrompts = ref([
  ...
  },
  {
    ...
    icon: Code,
    color: 'cyan',
    favoritedAt: '2024-02-01'
  }
])

const sortOptions = [
  { value: 'newest', label: '收藏时间' },
...
const filteredPrompts = computed(() => {
  let result = favoritePrompts.value
...
const removeFavorite = (id: number) => {
  ...
  if (confirm('确定要取消收藏该提示词吗？')) {
    favoritePrompts.value = favoritePrompts.value.filter(p => p.id !== id)
  }
}
</script>
